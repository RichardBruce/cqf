#ifndef __CAPLET_PAYOFF_H__
#define __CAPLET_PAYOFF_H__

#include "payoff.h"

class caplet_payoff : public payoff
{
    public :
        caplet_payoff(const double d, const double e, const double p, const double t, const double n, 
            const double s, const double r, const double c = 12.0) 
            : start_date(d), eval_date(e), pay_date(p), tenor(t), notional(n), strike(s), rate(r), comp_freq(c) {  }
        ...
        double evaluate(const vector<double> &rates, const vector<double> &dates, const double *const mc_points) const
        {
            ...
            const double mc_rate = integrate_curve(rates, mc_points, rf_rate_idx, rate_idx, tenor_idx) / this->rate;//mc_points[(tenor_idx * rates.size()) + rate_idx];
            
            /* Convert to a discrete rate */
            const double discrete_rate = this->comp_freq * (std::exp(mc_rate / this->comp_freq) - 1.0);
            
            /* Calculate the payoff */
            const double payoff = this->tenor * std::max(discrete_rate - this->strike, 0.0);
    ...
    private :
        ...
        const double    notional;   /* The notional of the caplet       */
        const double    strike;     /* Strike of the caplet             */
        const double    rate;       /* The rate the caplet is struck on */
