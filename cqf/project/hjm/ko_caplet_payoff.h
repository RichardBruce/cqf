#ifndef __KO_CAPLET_PAYOFF_H__
#define __KO_CAPLET_PAYOFF_H__

#include "payoff.h"

class ko_caplet_payoff : public payoff
{
    public :
        ko_caplet_payoff(const double d, const double e, const double p, const double t, const double n, 
        ...
        double evaluate(const vector<double> &rates, const vector<double> &dates, const double *const mc_points) const
        {
            ...
            const double payoff = (mc_rate > barrier) ? 0.0 : (this->tenor * std::max(discrete_rate - this->strike, 0.0));
