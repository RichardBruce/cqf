#ifndef __FLOORLET_PAYOFF_H__
#define __FLOORLET_PAYOFF_H__

#include "payoff.h"

class floorlet_payoff : public payoff
{
    public :
        floorlet_payoff(const double d, const double e, const double p, const double t, const double n, 
        ...
        double evaluate(const vector<double> &rates, const vector<double> &dates, const double *const mc_points) const
        {
            ...
            const double payoff = this->tenor * std::max(this->strike - discrete_rate, 0.0);
